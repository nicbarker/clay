...
void RenderDropdownTextItem(int index) {
    CLAY(CLAY_IDI("ScrollContainerItem", index), Clay__AttachLayoutConfig(&dropdownTextItemLayout), Clay__AttachElementConfig((Clay_ElementConfigUnion) { &dropdownRectangleConfig }, CLAY__ELEMENT_CONFIG_TYPE_RECTANGLE)) { // We can save a lot of memory by re-using configs in loops rather than redefining them
...
//
        CLAY(CLAY_ID("RightPanel"), CLAY_LAYOUT(.layoutDirection = CLAY_TOP_TO_BOTTOM, .sizing = { .width = CLAY_SIZING_GROW() }, .childGap = 16)) {
            ...
            CLAY(CLAY_ID("MainContent"), CLAY_LAYOUT(.sizing = { .width = CLAY_SIZING_GROW(), .height = CLAY_SIZING_GROW() }), CLAY_SCROLL(.vertical = true)) {
                CLAY(CLAY_ID("MainContentInner"), CLAY_LAYOUT(.layoutDirection = CLAY_TOP_TO_BOTTOM, .padding = {16, 16}, .childGap = 16), CLAY_RECTANGLE(.color = {200, 200, 255, 255})) {
                    CLAY(CLAY_ID("FloatingContainer"), CLAY_LAYOUT(.sizing = { .width = CLAY_SIZING_FIXED(300), .height = CLAY_SIZING_FIXED(300) }), CLAY_FLOATING(.zIndex = 1, .attachment = { CLAY_ATTACH_POINT_CENTER_TOP, CLAY_ATTACH_POINT_CENTER_TOP }, .offset = {0, -16})) {
                        CLAY(CLAY_ID("FloatingContainerBorder"), CLAY_LAYOUT(.sizing = { CLAY_SIZING_GROW(), CLAY_SIZING_GROW() }, .padding = { 16, 16 }), CLAY_BORDER_OUTSIDE(.color = {80, 80, 80, 255}, .width = 2)) {
                            CLAY(CLAY_ID("FloatingContainerBackground"), CLAY_LAYOUT(.sizing = { CLAY_SIZING_GROW(), CLAY_SIZING_GROW() }), CLAY_RECTANGLE(.color = {140,80, 200, 200})) {
